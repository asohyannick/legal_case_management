...
1. Set `auth_service.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../auth_service.Infrastructure/auth_service.Infrastructure.csproj -s auth_service.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../auth_service.Infrastructure/auth_service.Infrastructure.csproj -s auth_service.Web.csproj
```
