...
1. Set `payment_service.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../payment_service.Infrastructure/payment_service.Infrastructure.csproj -s payment_service.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../payment_service.Infrastructure/payment_service.Infrastructure.csproj -s payment_service.Web.csproj
```
